...
class HandEyeCalibrationNode(Node):
    def __init__(self):
        ...
        # 参数设置
        self.declare_parameter('aruco_dictionary', 'DICT_4X4_50')  # 使用 DICT_4X4_50
        self.declare_parameter('marker_length', 0.048)  # 标记边长，单位：米
        self.declare_parameter('calibration_samples', 8)
        self.declare_parameter('camera_fx', 854.132209)
        self.declare_parameter('camera_fy', 826.391640)
        self.declare_parameter('camera_cx', 474.667103)
        self.declare_parameter('camera_cy', 343.609130)
        self.declare_parameter(
            'camera_dist_coeffs',
            [-0.009184, -0.020229, -0.000388, -0.038870, 0.0]
        )
        self.declare_parameter('base_frame', 'base_link')
        self.declare_parameter('end_effector_frame', 'tool0')  # 末端执行器坐标系
        self.declare_parameter('camera_frame', 'camera_link')   # 相机坐标系
    ...
    def image_callback(self, msg):
        ...
        if ids is not None and len(ids) > 0:
            ...
            # 绘制坐标轴
            cv2.drawFrameAxes(cv_image, self.get_camera_matrix(), self.get_dist_coeffs(), rvec, tvec, 0.048)
    ...
    def handle_capture_pose(self, request, response):
        ...
        try:
            ...
            # 取逆，得到基座相对于末端执行器的变换
            T_base_to_gripper = np.linalg.inv(T_robot_ee)

            # 存储样本
            self.camera_poses.append(self.current_camera_pose)
            self.robot_poses.append(T_base_to_gripper)
            self.sample_count += 1
    ...
    def perform_calibration(self):
        ...
        for i in range(self.num_samples):
            T_base_to_gripper = self.robot_poses[i]
            T_cam = self.camera_poses[i]

            R_base2gripper.append(T_base_to_gripper[0:3, 0:3].astype(np.float64))
            t_base2gripper.append(T_base_to_gripper[0:3, 3].astype(np.float64))
        ...
        try:
            ...
            # OpenCV的calibrateHandEye返回的是相机相对于末端执行器的变换矩阵（T_cam_to_gripper）
            # 在 Eye-to-Hand 配置下，视为 T_eye_to_base
            R_eye_to_base = R_hand_eye
            t_eye_to_base = t_hand_eye
            ...
            # 构建 4x4 转换矩阵
            T_eye_to_base = np.vstack((
                np.hstack((R_eye_to_base, t_eye_to_base.reshape(3, 1))),
                np.array([0, 0, 0, 1])
            ))
            ...
            q = tf_transformations.quaternion_from_matrix(T_eye_to_base)
            transform_msg.transform.translation.x = T_eye_to_base[0, 3]
            transform_msg.transform.translation.y = T_eye_to_base[1, 3]
            transform_msg.transform.translation.z = T_eye_to_base[2, 3]
            ...
            self.get_logger().info(f"手眼标定静态变换已发布：'{self.camera_frame}' 相对于 '{self.base_frame}'")
